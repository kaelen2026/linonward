...
`apps/web` is the To C Web application and article experience: a second Next.js 16 App Router app,
single-language (`zh-CN`), `noindex`, served on port 3002 so it can run alongside `www` (3000) and
`api` (3001). It is not a standalone administration app. Its public routes read published content;
protected administration routes manage the article lifecycle and read health, Prometheus metrics,
and Tempo traces.
...
│   ├── articles/             # published article index and ID-addressed detail pages
